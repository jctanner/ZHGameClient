...
Write-Host "NO UI REFRESH - Testing if refresh breaks UI" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan
...
try {
    ...
    # SKIP REFRESH UI - Testing if this is what breaks the display
    Write-Host "    SKIPPING Skirmish.RefreshUI (testing)..." -ForegroundColor Yellow
    Start-Sleep -Seconds 1
    ...
    Write-Host "  Slot 0: $($respObj.result.slots[0].state) template=$($respObj.result.slots[0].template) at position $($respObj.result.slots[0].start_position + 1) (color: $($respObj.result.slots[0].color))" -ForegroundColor White
    Write-Host "  Slot 1: $($respObj.result.slots[1].state) template=$($respObj.result.slots[1].template) at position $($respObj.result.slots[1].start_position + 1) (color: $($respObj.result.slots[1].color))" -ForegroundColor White

    Write-Host "`n========================================" -ForegroundColor Green
    Write-Host "Configuration Complete (NO UI REFRESH)!" -ForegroundColor Green
    Write-Host "Check if player slots are visible in UI" -ForegroundColor Green
    Write-Host "========================================`n" -ForegroundColor Green
