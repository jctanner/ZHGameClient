...
- `B030`: Game crashes when starting autonomous mode via remote named pipe commands, but works fine when started via local bot-ui. Remote launch via `launch-game-wmi-session.ps1` successfully starts the game in Session 1, and remote configuration via `test-death-valley-1v1.ps1` successfully sets up the skirmish, but executing `start-autonomous-game.ps1` (which sends `Autonomy.Configure`, `Autonomy.SetMode`, `Autonomy.Resume`, `Skirmish.Start`, and clicks the start button) causes the game to crash during the menu-to-gameplay transition. Local workflow (launch from desktop + bot-ui autonomy start) works correctly. The crash occurs consistently when autonomy is initiated remotely, regardless of whether the game was launched remotely or locally.
  status: reproduced. Remote launch works, remote configuration works, but remote autonomy start crashes. Local autonomy start works. Session targeting is correct (game in Session 1), named pipe communication works, crash appears specific to the remote autonomy start sequence.
- `B031`: Zone expansion is blocked by failed black market placement attempts. The bot can become stuck at a low zone count (e.g., 3 zones when desired_zone_count=30) because the macro decision tree prioritizes black market construction over supply stash placement for zone expansion. When black market placement repeatedly fails with `construct_site_not_created` (no valid placement site found), the bot never reaches the supply stash build logic in the decision cascade. Observed with sprawl_multiplier=10, money=$41K+, current_zone_count=3, desired_zone_count=30, where the bot attempted and failed `Game.BuildBlackMarketSmart` on every macro tick while never attempting `Game.BuildSupplyStashSmart` for zone expansion. The bot built 14 black markets (some succeeded earlier when space was available) but placement failures in developed zones blocked progression to the zone expansion logic that requires `stashZoneCount < desiredZoneCount`.
  status: code changed via TDD approach. Created new policy function `AIControlAdapterIsZoneExpansionUrgent` with unit tests (4 test cases covering large gap, small gap, at target, and threshold boundary). The macro decision cascade now checks zone expansion urgency (gap >= 5 zones) and skips black market priority when expansion is urgent. This allows the cascade to reach production building or other macro decisions instead of infinite black market retry loops. Needs gameplay validation to confirm bot expands from 3 to 30 zones without getting stuck on failed black market placement.
- `B032`: Black market placement fails with `construct_site_not_created` even when map has plenty of room for expansion. Observed in same session as B031 where bot successfully built 14 black markets early but then could not place any more, despite having only 3/30 zones developed, $41K+ cash, and large unexplored map areas. The `Game.BuildBlackMarketSmart` placement algorithm appears to be limited to searching within already-developed zones which become saturated, rather than expanding search radius to undeveloped areas of the map. Potential root causes: (a) placement search constrained to active zone boundaries only, (b) spacing/proximity requirements too strict for dense base areas, (c) terrain pathfinding limitations preventing placement outside initial sprawl footprint, or (d) placement not coordinating with zone expansion to reserve sites in newly-claimed supply areas. This is the underlying cause that triggers B031's infinite retry behavior - B031's urgency fix works around the symptom (decision priority blocking) but doesn't address why placements fail when map space exists.
  status: newly identified root cause of B031. Needs investigation of smart placement search algorithm, particularly whether it searches beyond current zone boundaries and how it handles saturated base areas.
